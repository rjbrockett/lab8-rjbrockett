...
module MakeInterval (Endpoint : ORDERED_TYPE) = 
  struct
    ...
    let intersect (intvl1 : interval) (intvl2 : interval) : interval =
      ...
      | Empty -> Empty
      | Interval (x, y) ->
      if x > b || a > y then Empty
        else let lowBound = if x > a then  x else  a in
        let highBound =
        if y > b then  y else  b in
        Interval (lowBound, highBound)
      )
    end
    ;;
...
let intvl1 = IntInterval.create 1 6 ;;
let intvl2 = IntInterval.create 3 8 ;;
